...
class Functor f => Pretty f where
  prettyPrint :: a ~ String => f a -> String

pretty :: Pretty f => Expr f -> String
pretty = foldExpr prettyPrint

instance Pretty Lit where
  prettyPrint (Lit x) = show x

instance Pretty Add where
  prettyPrint (Add x y) = "(" ++ x ++ "+" ++ y ++ ")"

instance Pretty Mult where
  prettyPrint (Mult x y) = "(" ++ x ++ "*" ++ y ++ ")"

instance (Pretty f, Pretty g) => Pretty (f :+: g) where
  prettyPrint (Inl x) = prettyPrint x
  prettyPrint (Inr x) = prettyPrint x
